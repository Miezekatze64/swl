...
fn main() {
    ...
    type Generate =
        fn(Vec<intermediate::Inst>, &HashMap<String, usize>, &Vec<(String, String)>) -> String;
    let arch_result: Result<(Intrinsics, Generate), &'static str> = match target {
        Target::Linux => Ok((codegen_x86_64::intrinsics, codegen_x86_64_linux::generate)),
        Target::Bsd => Err("BSD"),
        Target::Mac => Err("MacOS"),
        Target::Windows => Ok((codegen_x86_64::intrinsics, codegen_x86_64_windows::generate)),
        Target::Wasm => Err("WASM"),
    };
    ...
    let filename: String = match pos_args.get(0) {
        ...
        None => {
            eprintln!(
                "{COLOR_RED}ERROR: <filename> not given{COLOR_RESET}\n{}",
                usage(arg0),
                COLOR_RED = color_red(),
                COLOR_RESET = color_reset()
            );
    ...
    // construct parser
    let mut parser = parser::Parser::new(contents, filename.clone(), verbose, target)
        .unwrap_or_else(|a| {
    ...
    if checked {
        if let Ok(mut ast) = a {
            ...
            let (intermediate, globals, externs) =
                intermediate::gen(ast.1, &mut HashMap::new(), &globals, aliases, 0, 1, true);
            if verbose > 2 {
            ...
            if !interpret {
                ...
                let time = SystemTime::now()
                    ...
                    .expect("not possible: backwards time...")
                    .as_secs_f32();
